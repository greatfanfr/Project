...
from update import test_inference, LocalUpdate_CVR
from models import CNNFashion_Mnist, client_model, ResNet18, Net2
from utils import init_seed, plot_results, get_dataset2, update_global_weights_CVR
...
if __name__ == '__main__':
    ...
    args = args_parser()
    m_name = "FedCVR"  # the variant of FedCVR with the average of local models as the global model
    # consider the convex combination as the local update
    # we also use the local ratio to update the global control instead of directly averaging all local controls
    # the performance should be equal to that of FedCVR5
    if args.dataset == 'mnist':
        penalty = 0.5  # gamma_i
    elif args.dataset == 'cifar10':
        penalty = 0.1
    else:
        raise ValueError("no other datasets are supported!!!")
    # eta_g = 15  # d_i^r
    local_ratio = 0.1  # the adaptive ratio
    ...
    for tt in range(1, run_num + 1):
        ...
        global_init = copy.deepcopy(global_weights)
        local_ctls = []
        ...
        for round_idx in tqdm(range(args.rounds)):
            ...
            for idx in idxs_users:
                ...
                #                          idxs_l=user_groups_l[idx], idxs_u=user_groups_u[idx], logger=logger
                #                          , gct_k=duals[idx])
                local_model = LocalUpdate_CVR(args=args, dataset=train_dataset, data_idxs=user_groups[idx],
                                              control_k=local_ctls[idx], global_weights=global_init, penalty=penalty,
                                              local_ratio=local_ratio)
                w, dl, loss, ctr_ratio = local_model.update_weights(global_round=round_idx)
                with torch.no_grad():
                    local_weights[idx] = w
                    local_ctls[idx] = dl
                    local_ctr_ratios[idx] = ctr_ratio

                local_losses.append(loss)

                # print("idx:",idx)
            # global_old = copy.deepcopy(global_weights)
            # global_weights, tau_w = update_global_weights(global_weights, local_weights, idxs_users, p_ratio, tau,
            #                                              args.lr)
            global_weights, global_ctl, global_init = update_global_weights_CVR(global_init=global_init,
                                                                                ...
                                                                                p_ratio=p_ratio,
                                                                                penalty=penalty,
                                                                                local_ctr_ratios=local_ctr_ratios)
    ...
    exp_name = args.exp_addName+args.dataset + "/" + m_name + "/" + str(args.model) \
               ...
               + ",penalty=" + str(penalty) + ",local_ratio" + str(local_ratio) \
