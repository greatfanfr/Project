...
from update import test_inference, LocalUpdate_Dyn
from models import CNNFashion_Mnist, client_model, ResNet18, Net2
from utils import init_seed, plot_results, update_global_weights_Dyn, get_dataset2
...
if __name__ == '__main__':
    ...
    for tt in range(1, run_num + 1):
        ...
        for idx in range(args.num_users):
            ...
            duals.append(copy.deepcopy(initial))
        ...
        for round_idx in tqdm(range(args.rounds)):
            ...
            for idx in idxs_users:
                ...
                #                          , gct_k=duals[idx])
                local_model = LocalUpdate_Dyn(args=args, dataset=train_dataset, data_idxs=user_groups[idx],
                                              dual_k=duals[idx], global_weights=global_weights, penalty=penalty)
                w, dl, loss = local_model.update_weights(global_round=round_idx)
                with torch.no_grad():
                    local_weights[idx] = w
                    duals[idx] = dl
            ...
            #                                              args.lr)
            global_weights = update_global_weights_Dyn(global_weights=global_weights,
